...
async function adminIndex() {
  ...
  let finalHTML = html.replace("__CA_TOKEN__", encryptionUtils.signUiToken());
  finalHTML = finalHTML.replace("__PAGE_TITLE__", config.get("appName"));
    ...
    .guard(
      {
        ...
        beforeHandle({ headers, status }) {
          ...
          if (
            typeof token !== "string" ||
            !encryptionUtils.verifyUiToken(token)
          )
