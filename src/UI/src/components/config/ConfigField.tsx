...
/**
 * One editable config row: key (+ doc hint when the server provides one),
 * format/base/encrypted badges, value input, encryption switch, delete and
 * undo affordances.
 *
...
export function ConfigField({
  ...
  return (
    <div
      ...
      className={cn("py-2.5", isDeleted && "bg-destructive/5")}
    >
          ...
          {isDeleted ? (
            ...
            <>
              {isBooleanField ? (
                ...
                <Input
                  value={
                    draft?.value ?? (entry.is_encrypted ? "" : entry.value)
                  }
                  placeholder={
      ...
      {entry.doc ? (
        <p className="mt-2 text-xs leading-snug break-words text-muted-foreground">
          {entry.doc}
        </p>
      ) : null}
    </div>
